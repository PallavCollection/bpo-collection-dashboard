...
def authenticate_user(email, password):
    return email in ["jjagarbattiudyog@gmail.com", "option6@example.com"] and password == "Sanu@1998"
...
if not st.session_state.authenticated:
    ...
    for i in range(config['process_count']):
        ...
        st.sidebar.markdown(f"### 🗂️ Manage Files for {process_name}")
        for label, path in [("🟡 Allocation", alloc_path), ("🔵 Paid Current", paid_current_path), ("🔴 Paid Previous", paid_prev_path)]:
            if os.path.exists(path):
                st.sidebar.write(f"{label} File Found ✅")
                if st.sidebar.button(f"🗑️ Delete {label} File", key=path):
                    os.remove(path)
                    st.sidebar.success(f"{label} file deleted.")
                    st.rerun()
            else:
                st.sidebar.write(f"{label} File Missing ❌")
